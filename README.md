...
```bash
unzip director-applicant-search-rd-kit.zip
cd director-applicant-search
chmod +x load.sh
...
```bash
docker exec -i director-applicant-rd-postgres \
  psql -U postgres -d director_applicant_rd -v ON_ERROR_STOP=1 < 03_baseline_queries.sql
```

Record latency and plans before changing anything. Re-run after each experiment.
